...
if ( $logo_img || $overlay_logo ) {
	...
	if ( $logo_img && ! $overlay_logo ) {
		...
		$output .= '<h1 class="logo-wrap">
		<a href="' . esc_url( $logo_url ) . '" rel="home" class="main-logo">';		
		//$output .= '<img ' . wpex_parse_attrs( $img_attrs ) . ' />';
		$output .= '<span>' . esc_attr( $logo_title ) . '</span><img ' . wpex_parse_attrs( $img_attrs ) . ' />';
		$output .= '</a>		
		<a href="' . esc_url( $logo_url ) . '" rel="home" class="mobile-logo">';		
		//$output .= '<img ' . wpex_parse_attrs( $img_attrs ) . ' />';
		$output .= '<span>' . esc_attr( $logo_title ) . '</span><img src="/wp-content/uploads/2018/12/avalere-mobile-logo.jpg" />';
		$output .= '</a>		
		</h1>';
